...
//-----------SD Module-----------//
//SPI Communiction: CS: Pin 53; DI: Pin 51; DO: Pin 50; CLK: Pin 52
//Power: 5V & GND
...
//-----------Keypad-----------//
// Keypad Pin from left to right with keys facing you connection with Mega digital pins respectively 
...
void safeRemoveSD() {
  ...
  clearDisplayExceptTitle();
  lcd.setCursor(0, 2);
  lcd.print("Stopping logging...");
  delay(500);

  clearDisplayExceptTitle();
  lcd.setCursor(0, 2);
  lcd.print("Remove SD now");
  delay(3000);
  clearDisplayExceptTitle();
  sdInserted = false;
}
    ...
    while (input.length() < 12) {
      ...
      if (key >= '0' && key <= '9') {
        ...
        for (int i = 0; i < input.length(); i++) {
          formatted += input[i];
          if (i == 3 || i == 5) formatted += '-'; // After YYYY and MM
          if (i == 7) formatted += ' ';           // After DD
          if (i == 9) formatted += ':';           // After hh
        }
